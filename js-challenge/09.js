...
function playWithJson(args) {
    ...
    args.forEach(shop => {
        ...
        shop.grand_total = grandTotal
        modified.push(shop)
    })

    let february = modified.filter(shop => shop.created_at.charAt(6) === '2')
    let purchasedByAri = modified.filter(shop => shop.customer.name === 'Ari')
    let grandTotalLowerThan = () => {
        let peopleName = []
        let newArr = []
        let arr = modified.filter(shop => shop.grand_total < 300000).forEach(shop => {
            peopleName.push(shop.customer.name)
        })

        for (let value of peopleName) {
    ...
    return {
        all_purchase_in_february: february,
        purchased_by_ari: purchasedByAri,
        grand_total_lower_than_300000: grandTotalLowerThan()
    }
}

console.log(playWithJson(json))